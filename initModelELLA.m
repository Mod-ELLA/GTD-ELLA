...
%   isRL : If the tasks are RL
%             RLParams.u - Vector according to GTD(0) ;
%             RLParams.iter = Total numbel of actions done for in learning 
%                             of the task
%             RLParams.expectation = % Expectation of the matrix 
%                               $1/k \sum_{i=1}^k \phi_i (\phi_i - gamma
%                               \phi')^{T}$ where k is the number of 
%                               iterations for this task so far
...
function model = initModelELLA(options)
    ...
    if ~ismember('isRL',fields(options))
	isRL = false;
    else
	isRL = options.isRL;
    end
