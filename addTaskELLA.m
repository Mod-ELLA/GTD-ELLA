...
% task : this contains the parameters of the RL task
...
function [model,exp] = addTaskELLA(model,task,lookUpTable,justEncode)
    taskid = task.t;
    % Train the new task add it to the model and return the D matrix with
    % the s from sparse coding
    if length(lookUpTable)==1 && lookUpTable{1}==-1
    [model.S(:,taskid), model.theta{taskid}, model.D{taskid}, model.taskSpecific{taskid}, model.T, model.RLParams] = encodeTaskELLARL(model,task);    

    else
    [model.S(:,taskid), model.theta{taskid}, model.D{taskid}, model.taskSpecific{taskid}, model.T, model.RLParams,exp] = encodeTaskELLARL(model,task,lookUpTable);
